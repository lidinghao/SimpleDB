...
public class BTreeRootPtrPage implements Page {
	...
	/**
	 * Constructor. Construct the BTreeRootPtrPage from a set of bytes of data
	 * read from disk. The format of an BTreeRootPtrPage is an integer for the
	 * page number of the root node, followed by a byte to encode the category
	 * of the root page (either leaf or internal), followed by an integer for
	...
	/**
	 * Generates a byte array representing the contents of this root pointer
	 * page. Used to serialize this root pointer page to disk. The invariant
	 * here is that it should be possible to pass the byte array generated by
	 * getPageData to the BTreeRootPtrPage constructor and have it produce an
	 * identical BTreeRootPtrPage object.
	 *
	 * @return A byte array corresponding to the bytes of this root pointer
	 *         page.
	 */
	 ...
	 * BTreeRootPtrPage. Used to add new, empty pages to the file. Passing the
	 * results of this method to the BTreeRootPtrPage constructor will create a
	 * BTreeRootPtrPage with no valid entries in it.
	 *
