...
public class LogicalPlan {
	 ...
	 * supplied to {@link Catalog#getDatabaseFile(int)}). Aliases are added as
	 * base tables are added via {@link #addScan}.
	 * 
	 * @param alias
	...
	public static void main(String argv[]) {
		...
		/*
		 * SeqScan ss1 = new SeqScan(tid, table1.getId(), "t1"); SeqScan ss2 =
		 * new SeqScan(tid, table2.getId(), "t2");
		 * 
		 * // create a filter for the where condition Filter sf1 = new Filter(
		 * new Predicate(0, Predicate.Op.GREATER_THAN, new IntField(1)), ss1);
		 * 
		 * JoinPredicate p = new JoinPredicate(1, Predicate.Op.EQUALS, 1); Join
		 * j = new Join(p, sf1, ss2);
		 */
		DbIterator j = null;
		try {
