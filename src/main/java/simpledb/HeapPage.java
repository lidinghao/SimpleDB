...
public class HeapPage implements Page {
	...
	public HeapPage(HeapPageId id, byte[] data) throws IOException {
		this.pid = id;
		this.tDesc = Database.getCatalog().getTupleDesc(id.getTableId());
	...
	private int getNumTuples() {
		...
		// pid should have been set up correctly.
		int bufferPoolSize = Database.getBufferPool().getPageSize();
		int tupleSize = Database.getCatalog().getTupleDesc(getId().getTableId()).getSize();
		return bufferPoolSize * 8 / (tupleSize * 8 + 1);
	}
	...
	private int getHeaderSize() {
		...
		// some code goes here
		// ceiling(no. tuple slots / 8)
		int headerSize = (getNumTuples() + 7) / 8; // ceiling
		return headerSize;
	...
	private Tuple readNextTuple(DataInputStream dis, int slotId) throws NoSuchElementException {
		...
		t.setRecordId(rid);
		try {
			for (int j = 0; j < tDesc.numFields(); j++) {
				Field f = tDesc.getFieldType(j).parse(dis);
				t.setField(j, f);
			}
	...
	public byte[] getPageData() {
		...
		// padding
		int zerolen = BufferPool.getPageSize() - (header.length + tDesc.getSize() * tuples.length); // -
																									// numSlots
																									// *
																									// td.getSize();
	...
	public void deleteTuple(Tuple t) throws DbException {
		...
		if (t.getRecordId().getPageId().equals(getId()) == false) {
	...
	public void insertTuple(Tuple t) throws DbException {
		...
		if (tDesc.equals(t.getTupleDesc()) == false) {
