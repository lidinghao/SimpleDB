...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderGreedyJoins(
        ...
        //base table
        //Set<Set<LogicalJoinNode>> sets = enumerateSubsets(joins, 1);

//        for (Set<LogicalJoinNode> set : sets) {
//            double bestSoFar = Double.POSITIVE_INFINITY;
//            LogicalJoinNode nodeToRemove = ((LogicalJoinNode) set.toArray()[0]);
...
//        }
        ...
        Set<LogicalJoinNode> leftJoins = new HashSet<LogicalJoinNode>(joins);

        while (leftJoins.size() > 0) {

            double bestSoFar = Double.POSITIVE_INFINITY;
            CostCard bestCostCard = null;
            LogicalJoinNode bestJoinNode = null;

            for (LogicalJoinNode join : leftJoins) {
                CostCard costCard = costGreedyJoin(join,
                        plan,
                        planCardinalities,
                        planCosts,
                        stats,
            ...
            plan.add(bestJoinNode);
            planCardinalities.add(bestCostCard.card);
            planCosts.add(bestSoFar);

            leftJoins.remove(bestJoinNode);
        }
    ...
    public Vector<LogicalJoinNode> orderDynamicProgrammingJoins(
        ...
        for (int i = 2; i <= joins.size(); i++) {
            Set<Set<LogicalJoinNode>> currentSets = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> set : currentSets) {
                double bestSoFar = Double.POSITIVE_INFINITY;
                Vector<LogicalJoinNode> bestPlan = new Vector<>();
                CostCard bestCostCard = null ;
                Vector vec = new Vector(set);
                Set<Set<LogicalJoinNode>> subSets = enumerateSubsets(vec, i-1);
                for (Set<LogicalJoinNode> subSet : subSets) {
                    //get best subplan from plancache
                    Set<LogicalJoinNode> setClone = (Set<LogicalJoinNode>) ((HashSet<LogicalJoinNode>) set).clone();
                    setClone.removeAll(subSet);
                    LogicalJoinNode joinNodeToRemove = (LogicalJoinNode) (setClone.toArray())[0];
                    CostCard costCard = computeCostAndCardOfSubplan(stats,
                            filterSelectivities,
                            joinNodeToRemove,
                            set,
                    ...
                    if (costCard != null && costCard.cost < bestSoFar) {
                        bestSoFar = costCard.cost;
        ...
        return planCache.getOrder(joinSet); //replace me
    }
