...
/**
 * Tests running concurrent transactions. You do not need to pass this test
 * until lab3.
 */
public class TransactionTest extends SimpleDbTestBase {
	...
	private static class XactionTester extends Thread {
		...
		public void run() {
			try {
				// Try to increment the value until we manage to successfully
				// commit
				while (true) {
					...
					try {
						...
						// race the other threads to finish the transaction: one
						// will win
						q1.close();
						...
						// set up a Set with a tuple that is one higher than the
						// old one.
