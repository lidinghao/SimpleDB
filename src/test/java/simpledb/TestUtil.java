...
public class TestUtil {
	 ...
	 * @param tupdata
	 *            an array such that the ith element the jth tuple lives in slot
	 *            j * width + i
	 * @require tupdata.length % width == 0
	 * @throws DbException
	 *             if we encounter an error creating the TupleIterator
	 */
	...
	/**
	 * Check to see if every tuple in expected matches <b>some</b> tuple in
	 * actual via compareTuples. Note that actual may be a superset. If not,
	 * throw an assertion.
	 */
	 ...
	 * thread.
	 *
	 * @return a handle to the Thread that will attempt lock acquisition after
	 *         it has been started
	 */
	static class LockGrabber extends Thread {
		...
		/**
		 * @param tid
		 *            the transaction on whose behalf we want to acquire the
		 *            lock
		 * @param pid
		 *            the page over which we want to acquire the lock
		 * @param perm
		 *            the desired lock permissions
		 */
