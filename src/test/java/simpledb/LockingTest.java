...
public class LockingTest extends TestUtil.CreateHeapFile {
	...
	public void setUp() throws Exception {
		...
		// test cases
		bp.getPage(tid, p0, Permissions.READ_WRITE).markPageDirty(true, tid);
		bp.getPage(tid, p1, Permissions.READ_WRITE).markPageDirty(true, tid);
		bp.getPage(tid, p2, Permissions.READ_WRITE).markPageDirty(true, tid);
		bp.flushAllPages();
		bp = Database.resetBufferPool(BufferPool.DEFAULT_PAGES);
	}
	 ...
	 * @param pid1
	 *            the first page to lock over
	 * @param perm1
	 *            the type of lock for the first page
	 * @param tid2
	 *            the second transaction Id
	 * @param pid2
	 *            the second page to lock over
	 * @param perm2
	 *            the type of lock for the second page
	 * @param expected
	 *            true if we expect the second acquisition to succeed; false
	 *            otherwise
	 */
	 ...
	 * @param expected
	 *            true if we expect the acquisition to succeed; false otherwise
	 */
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires two read
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires a read lock
	 * and a write lock on the same page, in that order.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires a write
	 * lock and a read lock on the same page, in that order.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires a read lock
	 * and a write lock on different pages.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires write locks
	 * on different pages.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires read locks
	 * on different pages.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Attempt lock
	 * upgrade.
	 */
	@Test
